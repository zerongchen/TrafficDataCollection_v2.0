...
import com.aotain.tdc.dao.traffic.IpAddressDao;
...
public class IpAddressService {
	 ...
	 CommonCache commonCache;
		...
		public void getTableColumns(BaseDTO dto) {
			dto.setQueryDestIp(dto.getQueryDestIp().toLowerCase());
			dto.setResultList(ipAddressDao.getTableColumns(dto));
			...
			for(int i = 0; i < dto.getResultList().size(); i++){
				...
				try{
					...
					//result.put("FLOW_RATE",String.format("%.2f",Double.parseDouble(result.get("FLOW").toString())/(particle*Long.parseLong(result.get("COUNTNUM").toString()))/1024/1024*8));
					//result.put("FLOWUP_RATE",String.format("%.2f",Double.parseDouble(result.get("FLOW_UP").toString())/(particle*Long.parseLong(result.get("COUNTNUM").toString()))/1024/1024*8));
					//result.put("FLOWDN_RATE",String.format("%.2f",Double.parseDouble(result.get("FLOW_DN").toString())/(particle*Long.parseLong(result.get("COUNTNUM").toString()))/1024/1024*8));
			...
			dto.setTotalCounts(ipAddressDao.getTableColumnsTotalCounts(dto));
		}
		
		@SuppressWarnings("rawtypes")
		public void getTableTrendColumns(BaseDTO dto) throws ExecutionException {
			//if(!dto.getQueryDetailsIdStr().equals("0")){
			//	dto.setQueryDestIp(dto.getQueryDetailsIdStr());
			//}
			dto.setQueryDestIp(dto.getQueryDestIp().toLowerCase());
			int particle = getParticle(dto) ;
			List<HashMap<String,Object>> listMapTimeP1  = ipAddressDao.getTimeParticleList(dto);
			...
			for(HashMap<String,Object> hashmap : listMapTimeP2){  
				...
				List<HashMap<String,Object>> listPFS = ipAddressDao.getFlowTrendByIdList(dto);
