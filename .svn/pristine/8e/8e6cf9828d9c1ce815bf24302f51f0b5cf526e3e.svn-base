...
<head>
<meta charset="UTF-8">
<title>业务配置--业务配置--业务信息管理_查看业务信息</title>
...
<script>
        ...
        function backhome()
		{
			window.location=ctx + "/serviceConfiguration/bizManage/index.do";
		}
		...
		$(function(){
	        ...
	        $("#del001").on("click", function(e){
	        	if(confirm("你确定要删除所选数据吗？")){
	    			...
	    			$.ajax({
	    				url : ctx + '/serviceConfiguration/bizManage/delBizInfos.do',
	    				type : "POST",
	    				data : postData,
	    				contentType : clientType,
	    				dataType : "json",
			...
			$("#popup02 .windowform .confirm button:eq(0)").on("click", function(e){
				...
				$.ajax({
					...
					data : info,
					dataType : 'json',
					url : '${ctx}/serviceConfiguration/bizManage/updateBizInfo.do',
			...
			$("#editForm .confirm button:eq(0)").on("click", function(e){
				...
				info.resourcemark = 1;
				//info.serverBuildId = $("#edit_SERVERBUILD_SERVERROOM_ID").val().split("-")[0];
				//info.serverRoomId = $("#edit_SERVERBUILD_SERVERROOM_ID").val().split("-")[1];
				$.ajax({
					type : "POST",
					data : info,
					dataType : 'json',
        ...
        function search(){
			...
			$.ajax({
				url : ctx + '/serviceConfiguration/bizManage/getBizInfo.do',
				type : "POST",
				data : postData,
				contentType : clientType,
				dataType : "json",
