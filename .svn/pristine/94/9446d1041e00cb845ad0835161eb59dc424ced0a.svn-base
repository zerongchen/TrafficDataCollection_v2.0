...
import com.aotain.tdc.service.businessAnalysis.CarrierProductAnalysisService;
import com.aotain.tdc.service.common.CommonService;

@Controller
@RequestMapping(value = "/traffic/businessAnalysis/carrierProduct")
public class CarrierProductController {
	
	@Autowired
	private CarrierProductAnalysisService carrierProductAnalysisService;
	...
	public String index(HttpServletRequest request){
		...
		return "businessAnalysis/carrierProduct";
	}
	
	 //查询统计数据
    @Secured({"ROLE_PRODUCT_ANALYSIS_CARRIERPRODUCT_QUERY"})
	...
	public PageResult<HashMap<String, String>> initStaTable(BaseDTO dto){
		...
		carrierProductAnalysisService.getTableColumns(dto);
    ...
    @Secured({"ROLE_PRODUCT_ANALYSIS_CARRIERPRODUCT_EXPORT"})
    @SuppressWarnings("unchecked")
	...
	public void exportStatistic(BaseModel<HashMap<String, String>> baseModel, BaseDTO dto, HttpServletResponse response,HttpServletRequest request) throws UnsupportedEncodingException{
		...
		String fileName = ModuleType.CARRIERPRODUCT_BUSINESS_ANALYSIS.getDescription()
