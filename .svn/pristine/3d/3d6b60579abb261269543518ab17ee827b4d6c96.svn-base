...
import com.aotain.tdc.dao.quality.ProductQualityDao;
...
public class ProductQualityService {
	...
	public void getTableColumns(BaseDTO dto) {		
		dto.setResultList(productQualityDao.getTableColumns(dto));
		...
		dto.setTotalCounts(productQualityDao.getTableColumnsTotalCounts(dto));
		if(dto.getTotalCounts() == null) dto.setTotalCounts(0);
	}

	public void getTrendTableColumns(BaseDTO dto) {
		dto.setResultList(productQualityDao.getTrenTableColumns(dto));
		for(int i = 0; i < dto.getResultList().size(); i++){
			@SuppressWarnings("unchecked")
			HashMap<String, Object> result = (HashMap<String, Object>) dto.getResultList().get(i);
			try{
		...
		dto.setTotalCounts(productQualityDao.getTrendTableColumnsTotalCounts(dto));
		if(dto.getTotalCounts() == null) dto.setTotalCounts(0);
	}

	public void getLayerTrendTableColumns(BaseDTO dto) {
		dto.setResultList(productQualityDao.getLayerTrenTableColumns(dto));
		for(int i = 0; i < dto.getResultList().size(); i++){
			@SuppressWarnings("unchecked")
			HashMap<String, Object> result = (HashMap<String, Object>) dto.getResultList().get(i);
			try{
		...
		dto.setTotalCounts(productQualityDao.getLayerTrendTableColumnsTotalCounts(dto));
		if(dto.getTotalCounts() == null) dto.setTotalCounts(0);
	}
}
