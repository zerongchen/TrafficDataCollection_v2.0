...
import com.aotain.tdc.service.businessAnalysis.RegionProductAnalysisService;
import com.aotain.tdc.service.common.CommonService;

@Controller
@RequestMapping(value = "/traffic/businessAnalysis/regionProduct")
public class RegionProductController {
	
	@Autowired
	private RegionProductAnalysisService regionProductAnalysisService;
	...
	public String index(HttpServletRequest request){
		...
		return "businessAnalysis/regionProduct";
	}
	
	 //查询统计数据
    @Secured({"ROLE_PRODUCT_ANALYSIS_REGIONPRODUCT_QUERY"})
	...
	public PageResult<HashMap<String, String>> initStaTable(BaseDTO dto){
		...
		regionProductAnalysisService.getTableColumns(dto);
    ...
    @Secured({"ROLE_PRODUCT_ANALYSIS_REGIONPRODUCT_EXPORT"})
    @SuppressWarnings("unchecked")
	...
	public void exportStatistic(BaseModel<HashMap<String, String>> baseModel, BaseDTO dto, HttpServletResponse response,HttpServletRequest request) throws UnsupportedEncodingException{
		...
		String fileName = ModuleType.REGIONPRODUCT_BUSINESS_ANALYSIS.getDescription()
