...
<c:set var="getTableStaColumnData"  value="${ctx}/common/TOP_PAGE_ANALYSIS/getTableColumnData.do"  scope="page" />
<c:set var="updateStaColumnUser"  value="${ctx}/common/TOP_PAGE_ANALYSIS/updateColumnUser.do" scope="request" />
<c:set var="initTable" value="${ctx}/traffic/businessAnalysis/topPageAnalysis/initTable.do" scope="request" />
<c:set var="exportTable" value="${ctx}/traffic/businessAnalysis/topPageAnalysis/exportTable.do" scope="request" />

<!DOCTYPE html>
<html lang="en">
<head>
<title>中移动流量采集监测系统-业务分析-TOP页面分析</title>
    ...
    <div class="con regional">
        <div class="frame">
            <div class="title">
                ...
                <span>TOP页面分析</span>
                <sec:authorize ifAllGranted="ROLE_PRODUCT_ANALYSIS_TOPPAGE_QUERY">
                <button class="fr drop_down">查询</button>
                </sec:authorize>
                <sec:authorize ifAllGranted="ROLE_PRODUCT_ANALYSIS_TOPPAGE_EXPORT">
...
<script type="text/javascript">
    ...
    <script src="${scriptPath}/view/businessAnalysis/topPageAnalysis.js" type="text/javascript"></script>
</body>
