...
<head>
<meta charset="UTF-8">
<title>业务配置--业务配置--基地资源池信息管理</title>
...
var setting = {
		...
		if(treeNode.level != 0){   
			...
			if (editBtn) {
				editBtn.bind("click", function(){
					...
					window.location="${ctx}/serviceConfiguration/baseResPool/detail.do?bizId="+treeNode.id;
					return false;
				});
			}
		}
	...
	function beforeRemove(treeId, treeNode) {
		...
		if(confirm("是否确认删除？")){
			...
			$.ajax({
				url : ctx + '/serviceConfiguration/baseResPool/delBizInfo.do',
				type : "POST",
				data : postData,
...
<body>
	...
	<div class="con back_search">
		<div class="frame">
			<div class="title">
				<i></i> > <span class="blue">业务配置</span> / <span class="blue">业务配置</span> / <span>基地资源池信息管理</span>
				<!-- 
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$("#popup02 .windowform .confirm button:eq(0)").on("click", function(e){
				...
				$.ajax({
					...
					data : info,
					dataType : 'json',
					url : '${ctx}/serviceConfiguration/baseResPool/insertBizInfo.do',
		...
		window.operateEvents = {
				'click .oper_edit' : function(e, value, row, index) {
					e.preventDefault();
					window.location="${ctx}/serviceConfiguration/baseResPool/detail.do?bizId="+row.BIZ_ID;
				}
			};
	</script>
	<script src="${scriptPath}/view/serviceConfiguration/BasePoolMange.js" type="text/javascript"></script> 
</body>
</html>