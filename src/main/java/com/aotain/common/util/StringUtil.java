...
public class StringUtil {
	...
	public static boolean isIPAddress(String s){
		...
		Pattern patern = Pattern.compile("^(((2[0-4]\\d)|(25[0-5]))|(1\\d{2})|([1-9]\\d)|(\\d))[.](((2[0-4]\\d)|(25[0-5]))|(1\\d{2})|([1-9]\\d)|(\\d))[.](((2[0-4]\\d)|(25[0-5]))|(1\\d{2})|([1-9]\\d)|(\\d))[.](((2[0-4]\\d)|(25[0-5]))|(1\\d{2})|([1-9]\\d)|(\\d))$");
		Matcher matcher = patern.matcher(s);
		if(matcher.matches()){
			return true;
		} else {
...
/*		String emt = "1|admin|2013-09-14 12:14:35";
    ...
    public static String getEncoding(String str) {
        ...
        String s = new String(str.getBytes(encode), encode);   
        encode = "ISO-8859-1";   
        String s1 = new String(str.getBytes(encode), encode);   
        encode = "UTF-8";   
        String s2 = new String(str.getBytes(encode), encode);   
        encode = "GBK";   
        String s3 = new String(str.getBytes(encode), encode);
        int a =2 ;
    	}catch(Exception e ){e.printStackTrace();}
		return str;
    }
  /*  String queryWhere = logStoreExport.getQUERYWHERE()
	.replace("*#1#*", "select a.mdn Mobile,a.destinationurl,a.sourceip,a.sourceport,a.proxyip,a.proxyport,a.destinationip,a.destinationport,a.partnai,concat('T:',a.visittime) visittime,from_unixtime(")
	.replace("*#2#*", ".starttime) starttime, from_unixtime(b.endtime) endtime from (select * from logstore_web where 1=1")
	.replace("*#3#*", ") a left join (select distinct mdn,clientip,starttime,endtime from logstore_radius where partflag=2")
	.replace("*#4#*", ") b on a.mdn=b.mdn and a.sourceip=b.clientip and a.starttime>=b.starttime and a.starttime<=b.endtime")
	.replace("*#5#*", ") b on a.mdn=b.mdn and a.sourceip=b.clientip and b.starttime>=(a.starttime-4)");
String cmd = "impala-shell -q \""+queryWhere+"\" -B --print_header --output_delimiter \",\" -o "+fileurl;*/
    /**
    ...
    */
	...
	public static String getToken(Long long1, String userName)
    {
    	Date now = new Date();
        String s = long1 + "|" + userName + "|" + DateUtils.formatDateTime("yyyy-MM-dd HH:mm:ss", now);
    ...
    public static String formatDateString(String dateString) {
    	...
    	try {
    		if(!StringUtils.isNotBlank(dateString)){
    			throw new Exception("the date should not be blank");
    		}
    		switch (dateString.trim().length()) {
    		case 12://201703100935
    			returnString = new SimpleDateFormat("yyyy-MM-dd HH:mm").format(new SimpleDateFormat("yyyyMMddHHmm").parse(dateString)) ;
    			break;
    		case 10://2017031009
    			returnString = new SimpleDateFormat("yyyy-MM-dd HH").format(new SimpleDateFormat("yyyyMMddHH").parse(dateString));
    			break;
    		default://2017-04-10 09:35:00.0
    			returnString = new SimpleDateFormat("yyyy-MM-dd HH:mm").format(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(dateString));
    			break;
    		}
    	} catch (Exception e) {
    	}
