...
public class ClogAdviceAfter implements MethodInterceptor {
	...
	public Object invoke(MethodInvocation invocation) throws Throwable {
		...
		for (String strategyId : baseDTO.getQueryStrategyId().split(",")) {
			...
			case DEL:
				break;
			case STOP_OR_PLAY:
				if (baseDTO.getQueryStatus() == 1)
					clogBean.setOP_TYPE(OpType.PLAY.getType());
				if (baseDTO.getQueryStatus() == 1)
					clogBean.setOP_TYPE(OpType.STOP.getType());
