...
import com.aotain.tdc.dao.traffic.CarrierDao;
...
public class CarrierService {
	 ...
	 CarrierDao carrierDao;
		...
		public void getTableColumns(BaseDTO dto) {
			dto.setResultList(carrierDao.getTableColumns(dto));
			List<HashMap<String,Object>> listTimeParticleCounts  = carrierDao.getTimeParticleCounts(dto);
			...
			for(int i = 0; i < dto.getResultList().size(); i++){
				...
				String carrierId = result.get("CARRIER_ID") + "";
				for(HashMap<String,Object> hashmap : listTimeParticleCounts){  
		            if(hashmap.get("CARRIER_ID").toString().equals(carrierId)){  
		            	timeParticleCounts = Integer.parseInt(hashmap.get("COUNTNUMS")+""); 
		            }  
		        }  
				try{
					result.put("CARRIER_NAME", commonCache.getDicCarrierBeanCache().get(Long.parseLong(carrierId)).getCarrierName());
			...
			dto.setTotalCounts(carrierDao.getTableColumnsTotalCounts(dto));
		}
		
		@SuppressWarnings("rawtypes")
		public void getTableTrendColumns(BaseDTO dto) throws ExecutionException {
			//if(dto.getQueryDetailsId() != 0){
			//	dto.setQueryCarriers(dto.getQueryDetailsId()+"");
			//}
			int particle = getParticle(dto) ;
			List<HashMap<String,Object>> listMapTimeP1  = carrierDao.getTimeParticleList(dto);
			...
			for(HashMap<String,Object> hashmap : listMapTimeP2){  
				...
				List<HashMap<String,Object>> listPFS = carrierDao.getFlowTrendByIdList(dto);
