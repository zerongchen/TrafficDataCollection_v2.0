...
import com.aotain.tdc.dao.traffic.RegionDao;
...
public class RegionService {
	 ...
	 RegionDao regionDao;
		...
		public void getTableColumns(BaseDTO dto) {
			dto.setResultList(regionDao.getTableColumns(dto));
			List<HashMap<String,Object>> listTimeParticleCounts  = regionDao.getTimeParticleCounts(dto);
			...
			for(int i = 0; i < dto.getResultList().size(); i++){
				...
				String provinceId = result.get("PROVINCE_ID") + "";
				for(HashMap<String,Object> hashmap : listTimeParticleCounts){  
		            if(hashmap.get("PROVINCE_ID").toString().equals(provinceId)){  
		            	timeParticleCounts = Integer.parseInt(hashmap.get("COUNTNUMS")+""); 
		            }  
		        }  
				try{
					result.put("PROVINCE_NAME", commonCache.getDicSysPositionBeanCache().get(Long.parseLong(provinceId)).getPositionName());
			...
			//dto.setTotalCounts(regionDao.getTableColumnsTotalCounts(dto));
		}
		...
		public void getTableTrendColumns(BaseDTO dto) throws ExecutionException {
			...
			//}
			int particle = getParticle(dto) ;
			List<HashMap<String,Object>> listMapTimeP1  = regionDao.getTimeParticleList(dto);
			...
			for(HashMap<String,Object> hashmap : listMapTimeP2){  
				...
				List<HashMap<String,Object>> listPFS = regionDao.getFlowTrendByIdList(dto);
