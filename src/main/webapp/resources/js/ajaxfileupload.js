...
jQuery
		...
		createUploadForm : function(id, fileElementId, data) {
			...
			if (data) {
				...
				for(var i = 0; i < data.length; i++) { 
					jQuery('<input type="hidden" name="' + data[i][0] + '" value="'
									+ data[i][1] + '" />').appendTo(form);
				}
			}
			//     传入唯一file控件的ID
		...
		createUploadForms : function(id, fileElementId, data) {
			//create form	
			var formId = 'jUploadForm' + id;
			var fileId = 'jUploadFile' + id;
			var form = jQuery('<form  action="" method="POST" name="' + formId
			...
			for ( var i = 0; i < fileElementId.length; i += 1) {
				//按name取值  如： ["filename","giffilename"];  
				var oldElement = jQuery("input[name=" + fileElementId[i] + "]");
				jQuery(oldElement).attr('id', fileId);
				jQuery(oldElement).parent().appendTo(form);
			}
		...
		ajaxFilesUpload : function(s) {
			...
			s = jQuery.extend( {}, jQuery.ajaxSettings, s);
			var id = new Date().getTime();
			var form = jQuery.createUploadForms(id, s.fileElementId,
					(typeof (s.data) == 'undefined' ? false : s.data));
			var io = jQuery.createUploadIframe(id, s.secureuri);
			var frameId = 'jUploadFrame' + id;
			var formId = 'jUploadForm' + id;
