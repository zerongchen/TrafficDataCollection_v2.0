...
function openlayer(id){
	...
	$('#chart_4').show();
    $('#div_list').hide();
	statisticsObject.searchLayer(id);
	layer.open({
		title: '趋势分析',
        type: 1,
        area: ['1100px', '625px'],
...
function operateFormatter(value, row, index) {
    return '<a class="show_view " style="cursor:pointer;" onclick = "openlayer(\''+row.BIZ_ID+'\')">查看</a>';
}
function search() {
	getTableColumnData(getTableStaColumnDataUrl);
		...
		success : function(data) {
			if(data != null && data.rows != null){
				...
				$.each(resultData, function(i, item){
					if(i<=9){
						xAxisFlowData.push(item['PRODUCT_NAME']);
