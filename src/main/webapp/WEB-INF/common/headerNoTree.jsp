	...
	$(function(){
		$(".sidebar-collapse img").css("cursor","pointer").bind('click',function(){
			if($(".sidel").css("display") == "block"){
				...
				$("#c-right").css("width", document.documentElement.clientWidth-10-18);
				if ($('#statisticalTable').length > 0) {
					$("#statisticalTable").css("width", document.documentElement.clientWidth-10-18-50);
				}
				if ($('#trendTable').length > 0) {
					$("#trendTable").css("width", document.documentElement.clientWidth-10-18-50);
				}
				if ($('#appTable_up').length > 0) {
					$("#appTable_up").css("width", document.documentElement.clientWidth-10-18-50);
				}
				if ($('#appTable_dn').length > 0) {
					$("#appTable_dn").css("width", document.documentElement.clientWidth-10-18-50);
				}
				if ($('#appTable_statistical').length > 0) {
					$("#appTable_statistical").css("width", document.documentElement.clientWidth-10-18-50);
				}
				if ($('#appTable_trend').length > 0) {
					$("#appTable_trend").css("width", document.documentElement.clientWidth-10-18-50);
				}
			}else{
				$(".sidel").show();
